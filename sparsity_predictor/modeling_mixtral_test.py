...
class MixtralBLockSparseTop2MLP(nn.Module):
    def __init__(self, config: MixtralConfig, layer_idx: int, expert_idx: int):
        ...
        # 取v的top20%大小的index2
        self.k1 = int(self.ffn_dim * 0.1)  # 计算top 10%的大小
    ...
    def print_ratio(self):
        ...
        """ 
        print(f'{(sum(self.containment_ratio)/len(self.containment_ratio)):.4f}')
        if self.token_sum == 0:
    ...
    def get_ratio(self) -> float:
        ...
        if self.token_sum == 0:
            print("counting start....")
            # print(self.up_threshold.device)
            return sum(self.containment_ratio)/len(self.containment_ratio)
    ...
    def forward(self, hidden_states, routing_weights, preatt_score=None):
        ...
        if preatt_score != None:
            ...
            # 取current_hidden_states的top10%大小的index1
            _, index1 = torch.topk(current_hidden_states, self.k1, dim=-1)  # 获取top 10%的索引 
            _, index2 = torch.topk(v, self.k1*2, dim=-1)  # 获取top 20%的索引

            # 保存 index2包含 index1的比例
            # 将index1和index2转换为集合，计算交集的大小
            index1_set = set(index1.cpu().numpy().flatten())
            index2_set = set(index2.cpu().numpy().flatten())
            intersection_size = len(index1_set.intersection(index2_set))
            self.containment_ratio.append(intersection_size / len(index1_set))  # 计算包含比例
