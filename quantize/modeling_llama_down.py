...
def load_thresholds(threshold_path, use_average=True, zero = False):
    ...
    if zero:
        ...
        print(up_th)
        return
    up_th = torch.load(threshold_path, map_location='cuda')["down_proj_states_thresholds"]
...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        sparsity_prediction = up_result * activation
        up_proj_states = torch.where(sparsity_prediction.abs() > self.up_threshold.to(x.device), up_result, 0.0, )
