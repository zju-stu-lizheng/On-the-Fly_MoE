...
class LlamaMLP(nn.Module):
    def __init__(self, config, layer_idx = 0):
        ...
        self.up_threshold = up_th[self.layer_idx]
        self.count_sum = 0
        self.token_sum = 0
    ...
    def print_ratio(self):
        ...
        if self.token_sum == 0:
            ...
            return
        print(f'layer {self.layer_idx} ratio: {self.count_sum/self.token_sum:.4f}')
        self.count_sum = 0
        self.token_sum = 0
    ...
    def forward(self, x):
        ...
        if use_sp_type == "up":
            up_result = torch.where(up_result.abs() > self.up_threshold.to(x.device), up_result, 0.0, )
            ### Calculate actual preserved ratio
            true_ratio = (up_result != 0).sum().item()
        elif use_sp_type == "gate":
            activation = torch.where(activation.abs() > self.up_threshold.to(x.device), activation, 0.0, )
            ### Calculate actual preserved ratio
            true_ratio = (activation != 0).sum().item()
        
        self.count_sum += true_ratio
        self.token_sum += up_result.numel()
