...
with open('../path.json', 'r') as file:
    ...
    save_path = paths.get('deepseek_threshold','')
...
n_experts = len(model.model.layers[1].mlp.experts)
...
up_states = [[torch.zeros([accum_steps * batch_size * block_size , 1408], device=device_2) for _ in range(n_experts)] for _ in range(n_layers)]
gate_states = [[torch.zeros([accum_steps * batch_size * block_size , 1408], device=device_2) for _ in range(n_experts)] for _ in range(n_layers)]
down_states = [[torch.zeros([accum_steps * batch_size * block_size , 1408], device=device_2) for _ in range(n_experts)] for _ in range(n_layers)]
        ...
        for batch_idx in range(accum_steps):
            ...
            for layer_idx in range(n_layers):
                ...
                for expert_idx in range(n_experts):
                    ...
                    try:
                        states = model.model.layers[layer_idx].mlp.experts[expert_idx].up_proj_states.reshape(-1, 1408)
                        ...
                        states = model.model.layers[layer_idx].mlp.experts[expert_idx].gate_proj_states.reshape(-1, 1408)
                        gate_states[layer_idx][expert_idx][counts : counts+cur_counts, :] = states
                        # counts += cur_counts
                        states = model.model.layers[layer_idx].mlp.experts[expert_idx].down_proj_states.reshape(-1, 1408)
        ...
        for layer_idx in range(n_layers):   
            ...
            for expert_idx in range(n_experts):
                try:
                    ...
                    topk_num = int(useful_num * 1408 * sparsity_level)
...
for layer_idx in range(n_layers):
    if layer_idx == 0:
        up_proj_states_thresholds[layer_idx][expert_idx] = 0
        gate_proj_states_thresholds[layer_idx][expert_idx] = 0
        down_proj_states_thresholds[layer_idx][expert_idx] = 0
    for expert_idx in range(n_experts):
...
# up_proj_states_thresholds_2 = [[torch.zeros(1408) for _ in range(n_experts)] for _ in range(n_layers)]
...
#                 for expert_idx in range(n_experts):
#                     counts = all_counts[layer_idx * n_experts + expert_idx]
#                     states = model.model.layers[layer_idx].mlp.experts[expert_idx].up_proj_states.reshape(-1, states.size(-1))
#                     cur_counts = states.size(0)
