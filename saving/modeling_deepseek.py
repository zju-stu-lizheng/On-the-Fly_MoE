...
sparsity_selection = "gate"
def set_profile_mode(profile_mode, s_selection="gate"):
    global profile_sparsity
    global sparsity_selection
    profile_sparsity = profile_mode
    sparsity_selection = s_selection
    print(f'using profile_sparsity: {profile_sparsity} {s_selection}')
up_th = None
...
class DeepseekV2MLP(nn.Module):
    def __init__(self, config, hidden_size=None, intermediate_size=None, layer_idx=0, expert_idx=0):
        super().__init__()
        self.config = config
        ...
        self.gate_proj_states = None
        self.up_proj_states = None
        self.down_proj_states = None
        self.count_sum = 0
        self.token_sum = 0
    ...
    def forward(self, x):
        ...
        if sparsity_selection == "gate":
            activation = torch.where(torch.abs(activation) > self.threshold.to(x.device),
                activation, 0.0, )
            true_ratio = (activation != 0).sum().item()
            current_hidden_states = up_result * activation
        elif sparsity_selection == "up":
            up_result = torch.where(torch.abs(up_result) > self.threshold.to(x.device),
                up_result, 0.0, )
            true_ratio = (up_result != 0).sum().item()
            current_hidden_states = up_result * activation
        else:
            current_hidden_states = up_result * activation
            current_hidden_states = torch.where(torch.abs(current_hidden_states) > self.threshold.to(x.device),
    ...
    def __init__(self, config, layer_idx):
        ...
        if hasattr(config, "ep_size") and config.ep_size > 1:
            ...
            self.experts = nn.ModuleList(
                [
                    (
                        DeepseekV2MLP(
                            config, intermediate_size=config.moe_intermediate_size, layer_idx=layer_idx,expert_idx=i
                        )
        ...
        if config.n_shared_experts is not None:
            ...
            self.shared_experts = DeepseekV2MLP(
                config=config, intermediate_size=intermediate_size, layer_idx=0,expert_idx=0
            )
...
class DeepseekV2DecoderLayer(nn.Module):
    def __init__(self, config: DeepseekV2Config, layer_idx: int):
        ...
        self.mlp = (
            ...
            else DeepseekV2MLP(config, layer_idx=layer_idx, expert_idx=0)
        )
