...
activation=[[] for _ in range(32)]
expert=[[] for _ in range(32)]
def set_th_sparsity(sparsity=50, dataset='c4_base'):
    """
    根据稀疏程度读取对应的阈值
    """
    global th
    filename = f'/home/lz/workspace/llama2-7b/HQQ/notebooks/threshold/{dataset}/th_{sparsity}.csv'
    th = read_csv_to_2d_list(filename)
    # print(th)

start_layer_idx = 1

def set_save_layer_idx(start_idx=1):
    """
    设置需要保存的层号
    """
    global start_layer_idx
...
class MixtralBLockSparseTop2MLP(nn.Module):
    ...
    def forward(self, hidden_states, routing_weights):
        ...
        acti = self.act_fn(self.w1(hidden_states)) 
        v = torch.abs(acti)
        ...
        if profile_mode:
            ...
            current_hidden_states = acti * self.w3(hidden_states)
        else:
            mask = (v >= th[self.layer_idx][self.expert_idx]).to(hidden_states.dtype)
            current_hidden_states = torch.mul(self.w3(hidden_states), mask) * torch.mul(acti, mask)
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        global activation
        global expert
        # nums_per_file = 500000
        nums_per_file = 5000
        if(self.layer_idx >= start_layer_idx - 1 and self.layer_idx < 31):
            activation[self.layer_idx + 1].extend([h for h in hidden_states])
        if(self.layer_idx >= start_layer_idx):
            expert[self.layer_idx].extend([e for e in router_logits])
        if self.layer_idx == 31 and (len(activation[1]) >= nums_per_file):
            global datai
            for i in range(1, 32):
                d=list(zip(activation[i],expert[i]))
                torch.save(d,f'merge/a2ef_{i}_{datai}.pth')
                datai+=1
                del(d)
                activation[i].clear()
                expert[i].clear()
